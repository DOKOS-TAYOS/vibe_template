...
- Keep the default loop cheap: targeted reads, targeted tests, Ruff first.
...
7. Run `bin\quality.cmd` or `./bin/quality.sh` while iterating.
8. Refactor while staying green.
9. Before finishing risky work, pushing, or merging, run `bin\quality.cmd --full` or `./bin/quality.sh --full`.
