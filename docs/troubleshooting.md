...
## `quality --full` Fails on Pyright

- check that `.venv` exists
- confirm the package imports from `src`
- keep function typing explicit

The cheap default `quality` command does not run Pyright. If only `quality --full` fails, the Ruff stage is already fine and the problem is isolated to typing or imports.
