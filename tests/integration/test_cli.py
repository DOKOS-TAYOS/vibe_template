...
def test_cli_quality_help_lists_cheap_and_full_options() -> None:
    completed_process = subprocess.run(
        [sys.executable, "-m", "project_name.cli", "quality", "--help"],
        check=False,
        capture_output=True,
        text=True,
        env=_cli_env(),
    ...
    assert completed_process.returncode == 0
    assert "--check-only" in completed_process.stdout
    assert "--full" in completed_process.stdout
