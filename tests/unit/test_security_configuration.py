...
def test_security_workflow_runs_supply_chain_and_code_scanning_checks() -> None:
    repo_root = Path(__file__).resolve().parents[2]
    ...
    assert "github/codeql-action/init@v4" in workflow_content
    assert "github/codeql-action/analyze@v4" in workflow_content
    assert "python scripts/run_template_command.py security" in workflow_content
    assert "security-events: write" in workflow_content
