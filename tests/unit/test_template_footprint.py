...
def test_template_docs_reflect_strong_ai_positioning() -> None:
    repo_root = Path(__file__).resolve().parents[2]
    readme = (repo_root / "README.md").read_text(encoding="utf-8")
    ai_instructions = (repo_root / "docs" / "docs_for_ai" / "project_ai_instructions.md").read_text(
        encoding="utf-8"
    )
    ai_guide = (repo_root / "docs" / "docs_for_ai" / "guide_for_ai_users.md").read_text(
        encoding="utf-8"
    )
    ai_status = (repo_root / "docs" / "docs_for_ai" / "status.md").read_text(encoding="utf-8")

    assert "High-rigor Python template for AI-assisted projects." in readme
    assert "best code quality possible" in ai_instructions
    assert "full verification" in ai_instructions
    assert "strong workflow" in ai_guide
    assert "Strong AI template baseline" in ai_status
