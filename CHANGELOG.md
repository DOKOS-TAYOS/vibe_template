...
- Bootstrap tests now ignore repo-local `.pytest_cache` folders when copying a template workspace, avoiding Windows permission failures in reused worktrees.
...
- This `cheap-template` branch now treats `quality` as a cheap Ruff-only default, with `quality --full` as the explicit gate for `pytest` plus `pyright`.
- The AI docs and top-level human docs were rewritten to reduce reading overhead and to steer agents toward targeted searches, shorter responses, and smaller default verification loops.
- CI defaults are lighter in the cheap template: the main matrix stays on Windows and Ubuntu but only for Python 3.12, and the fresh-copy smoke job now runs on Windows only.
- The fresh-copy smoke helper now calls `quality --full`, so the post-bootstrap gate still validates the complete verification path even though the day-to-day default is cheaper.
...
- CI now includes a dedicated fresh-copy template smoke job on Windows, using Python 3.12 to validate the real bootstrap-plus-full-quality flow end to end.
