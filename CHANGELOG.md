...
- Strong AI positioning for the template, centered on design-first delivery, TDD, explicit typing, and full verification before completion.
- Stronger AI workflow guidance in the compact AI docs set, without adding extra markdown sprawl.
...
- Template metadata, CLI messaging, and human docs now describe the repository as a high-rigor AI-assisted Python template instead of a generic vibe-coding baseline.
- The default workflow guidance now makes full verification the expected finish line for AI-driven work, while keeping the existing command surface stable.
- CI contract tests now explicitly defend the cross-platform full-quality gate and fresh-copy smoke coverage.
