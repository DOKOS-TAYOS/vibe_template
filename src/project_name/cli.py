...
def _build_parser() -> argparse.ArgumentParser:
    ...
    quality_parser = subparsers.add_parser(
        "quality",
        help="Run cheap Ruff checks by default; use --full to include pytest and pyright.",
    )
    quality_parser.add_argument(
        "--check-only",
        action="store_true",
        help="Run non-mutating Ruff checks instead of applying fixes.",
    )
    quality_parser.add_argument(
        "--full",
        action="store_true",
        help="Extend the cheap flow with pytest and pyright.",
    )
...
def _handle_quality(args: argparse.Namespace) -> int:
    commands = build_quality_commands(check_only=args.check_only, full=args.full)
